...
//	@RequestMapping(value = "/request/basic01", method = RequestMethod.POST)
	...
	// 컨트롤러 내의 메서드 타입을 void로 선언하면 요청이 들어온 URL값을 뷰 리졸버에게 전달합니다.
	@GetMapping("join")
	public void register() {
		System.out.println("/request/join: GET");
	}
	
	// 요청 URI 주소가 같더라도 전송 방식에 따라 맵핑을 다르게 하기 때문에 같은 주소를 사용하는 것이 가능합니다.
	// (GET -> 화면 처리, POST -> 입력값 처리)
	
	/*
	 	# 스프링에서 요청과 함께 전달된 데이터를 처리하는 방식
	 	
	 	1. 전통적인 jsp/servlet 방식의 파라미터 읽기 처리 방법. (사용은 가능하지만 거의 안 씀)
		...
		-------------------------------------------------------------------------------------------
	 	...
	 	2. @RequestParam 아노테이션을 이용한 요청 파라미터 처리. (자주 사용됨.)
