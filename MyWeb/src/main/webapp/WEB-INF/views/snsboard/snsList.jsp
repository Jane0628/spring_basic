			...
			<%@ include file="../include/footer.jsp" %>
								...
								<div class="modal-con col-sm-4 col-xs-6">
									<div class="modal-inner">
										...
										<div class="content-inner">
											<p id="snsContent">Lorem ipsum dolor sit amet, consectetur
				...
				<script>
					...
					function register() {
						...
						// 세션에서 현재 로그인 중인 사용자의 정보(아이디) 얻어오기
						const userId = '${sessionScope.login}';

						// JS로 첨부한 파일의 확장자 체크 (사진이 아닌 파일을 거르기 위함)
						...
						if (file !== 'jpg' && file !== 'jpeg' && file !== 'png' && file !== 'bmp' && file !== 'gif') {
							alert('이미지 파일(jpg, jpeg, png, bmp, gif)만 등록이 가능합니다. :(');
						...
						*/
						...
						// 우리는 파일 첨부 버튼이 하나고, id를 지목해서 가져오기 때문에 인덱스를 쓸 필요는 없다.
								...
								for (vo of list) {
									str +=
										...
										<div class="like-inner">
											<!--좋아요-->
											<img src="${pageContext.request.contextPath}/img/icon.jpg">
											<span>522</span>
										</div>
					...
					/*
						...
						스크롤을 비효율적으로 많이 움직여야할 수도 있습니다.
						서비스하는 형식에 맞는 페이징 방식을 적용하면 됩니다.
					*/
					window.onscroll = function () {
						if (!isFinish) {
							/*
								윈도우(device)의 높이와 현재 스크롤 위치값을 더한 뒤,
								문서(컨텐츠)의 높이와 비교해서 같아졌다면 로직을 수행.
								문서 높이 - 브라우저 창 높이 = 스크롤 창의 끝 높이 -> 새로운 내용 불러오기!
							*/
							if (window.innerHeight + window.scrollY >= document.body.scrollHeight) {
								// 사용자의 스크롤이 바닥에 닿았을 때, 페이지 변수의 값을 하나 올리고
								// reset 여부는 false를 줘서 누적해서 계속 불러온다.
								// 게시글을 한 번에 몇 개씩 불러올지는 PageVO의 cpp를 조정하면 된다.
								console.log('페이징 발동!');
					...
					function readURL(input) {
						if (input.files && input.files[0]) {
							...
							reader.onload = function (event) { //읽기 동작이 성공적으로 완료 되었을 때 실행되는 익명함수
								...
								document.getElementById('fileImg').setAttribute('src',
									event.target.result);
