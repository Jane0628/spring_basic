						...
						<form action="">
							<div class="form-group">
								...
								<div class="input-group">
									...
									<input type="text" class="form-control" id="userId" placeholder="아이디를 (영문포함 4~12자)">
							...
							<div class="form-group">
								...
								<label for="password">비밀번호</label> <input type="password" class="form-control"
									id="userPw" placeholder="비밀번호 (영 대/소문자, 숫자 조합 8~16자)"> <span id="msgPw"></span>
								<!--자바스크립트에서 추가-->
							</div>
							<div class="form-group">
								<label for="password-confrim">비밀번호 확인</label> <input type="password"
							...
							<div class="form-group email-form">
								...
								<div class="input-group">
									<input type="text" class="form-control" id="userEmail1" placeholder="이메일"> <select
			...
			<script>
				...
				document.getElementById('idCheckBtn').onclick = () => {
					...
					if (userId === '') {
						alert('아이디를 적어주세요! :(');
						return;
					} else if (document.getElementById('msgId').textContent === '부적합한 아이디입니다. :(') {
						alert('적합한 아이디 입력 후 진행해주세요!');
						return;
					}
					...
					/*
						...
						// 요청 정보를 헤더에 설정
						xhr.setRequestHeader('content-type', 'text/plain'); // 객체 보낼 때는 JSON으로 바꾸지만, 문자 하나만 보낼 때는 이렇게 보내도 무관하다.
						xhr.send(userId);
						...
						xhr.onload = () => {
							...
							if (xhr.response === 'available') {
								...
								document.getElementById("msgId").innerHTML = "중복되는 아이디입니다. :(";
							}
						}
					*/
					...
					*/
					...
					.then(res => {
						// fetch 함수를 통해 비동기 통신이 실행되고, 요청이 완료된 후 then()의 매개값으로 응답에 관련된 함수를 콜백 방식으로 전달합니다.
						// 함수의 매개변수를 선언하면 해당 매개변수로 응답에 관련된 전반적인 정보를 가진 응답 정보가 전달됩니다.
						console.log(res);
				...
				id.onkeyup = function () {
					/*
						...
						특정 규칙이 있는 문자열 집합을 규칙을 직접 지정하여 탐색하게 해주는 메타 문자입니다.
					*/
					/*test메서드를 통해 비교하며, 매칭되면 true, 아니면 false반*/
					var regex = /^[A-Za-z0-9+]{4,12}$/;
					if (regex.test(document.getElementById("userId").value)) {
						document.getElementById("userId").style.borderColor = "green";
						document.getElementById("msgId").innerHTML = "아이디 중복 체크를 진행해주세요!";
