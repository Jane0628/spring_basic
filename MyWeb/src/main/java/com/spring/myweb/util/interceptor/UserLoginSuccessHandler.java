...
// 인터셉터로 사용할 클래스는 HandlerInterceptor 인터페이스를 구현해야 합니다.
@Slf4j
public class UserLoginSuccessHandler implements HandlerInterceptor{
	
	// preHandle은 컨트롤러로 요청이 들어가기 전 처리해야할 로직을 작성.
	@Override
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
			throws Exception {
		log.info("나는 preHandle이다!");
		return HandlerInterceptor.super.preHandle(request, response, handler);
	}

	// postHandle은 컨트롤러를 나갈 때 공통 처리해야할 내용을 작성.
	// /userLogin이라는 요청이 마무리된 후 viewResolver로 전달이 되기 전
