...
public class FreeBoardControllerTest {
	...
	/*
	 	...
	 	- WebApplicationContext는 Spring에서 제공되는 servlet들을 사용할 수 있도록 정보를 저장하는 객체입니다.
	 */
...
//	private FreeBoardController controller;
	...
	// MockMvc는 웹 어플리케이션을 서버에 배포하지 않아도 스프링 MVC 동작을 구현할 수 있는 가상의 구조를 만들어줍니다.
...
//		this.mockMvc = MockMvcBuilders.standaloneSetup(controller).build();
	...
	void testList() throws Exception {
		ModelAndView mv =
		mockMvc.perform(MockMvcRequestBuilders.get("/freeboard/freeList")) 	// 가상의 겟 요청 생성
		.andReturn()														// 결과를 받아서
		.getModelAndView();													// ModelAndView를 만든다.
	...
	void testInsert() throws Exception {
		String viewPage = mockMvc.perform(MockMvcRequestBuilders.post("/freeboard/freeRegister")
	...
	@Test
	@DisplayName("3번 글 상세 보기 요청을 넣으면 컨트롤러는 DB에서 가지고 온 글 객체를 모델에 담아서 jsp로 이동시킬 것이다.")
	...
	void testContent() throws Exception {
		...
		System.out.println("Model Data : " + mv.getModelMap());
	...
	void testModify() throws Exception {
		String viewPage = mockMvc.perform(MockMvcRequestBuilders.post("/freeboard/modify")
																.param("bno", "4")
																.param("title", "글 제목 수정했당")
																.param("content", "내용 수정했당"))
						  .andReturn().getModelAndView().getViewName();
		System.out.println("ViewPage : " + viewPage);
		assertEquals("redirect:/freeboard/content?bno=4", viewPage);
	}
	...
	void testDelete() throws Exception {
		assertEquals("redirect:/freeboard/freeList", mockMvc.perform(MockMvcRequestBuilders.post("/freeboard/delete")
																						   .param("bno", "4"))
													 .andReturn().getModelAndView().getViewName());
	}
